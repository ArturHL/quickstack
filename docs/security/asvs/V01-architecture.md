...
| 1.1.5 | Verificar definicion y analisis de seguridad de la arquitectura de alto nivel de la aplicacion y todos los servicios remotos conectados | L2 | ✅ | **Implementacion:** Arquitectura documentada en ARCHITECTURE.md. Servicios externos: Aurora (BD), Twilio/SendGrid (notificaciones). Autenticacion nativa con Spring Security + JWT (sin IdP externo). Cada servicio evaluado por seguridad y compliance. |
...
| ID | Requisito | Nivel | Estado | Medida Implementada |
|----|-----------|-------|--------|---------------------|
| 1.6.1 | Verificar que existe una politica explicita para gestion de claves criptograficas y que el ciclo de vida de claves sigue un estandar de gestion como NIST SP 800-57 | L2 | ⏳ | **Implementacion:** JWT signing: par de claves RS256 generadas en backend, rotacion anual. Claves de BD: AWS Aurora maneja encryption at rest. Claves de API (Twilio, SendGrid): almacenadas en environment variables de AWS Lambda, rotadas trimestralmente. Documentacion de rotacion en SECURITY.md. |
| 1.6.2 | Verificar que los consumidores de servicios criptograficos protejan material de claves y otros secretos usando vaults de claves o alternativas basadas en API | L2 | ⏳ | **Implementacion:** Secretos en AWS Lambda Environment Variables (encrypted at rest) o AWS Secrets Manager. No hay secretos hardcodeados en codigo. `.env` en `.gitignore`. JWT private key almacenada como env var. Para produccion futura: considerar HashiCorp Vault o AWS Secrets Manager exclusivo. |
| 1.6.3 | Verificar que todas las claves y contrasenas son reemplazables y son parte de un proceso bien definido para re-cifrar datos sensibles | L2 | ⏳ | **Implementacion:** JWT signing keys: rotacion anual con periodo de gracia 7 dias con ambas claves validas durante transicion, actualizar AWS Lambda env. Database credentials: rotacion via AWS Secrets Manager + Terraform deploy. API keys: rotacion individual por servicio. Passwords en Argon2id (no requieren re-cifrado, solo re-hash en login). |
...
| 1.7.2 | Verificar que los logs se transmitan de forma segura a un sistema preferiblemente remoto para analisis, deteccion, alertas y escalamiento | L2 | ⏳ | **Implementacion MVP:** Logs a CloudWatch vía Lambda. **Produccion:** Exportar a servicio externo (Datadog, Logflare, o Grafana Cloud) via HTTPS. Alertas configuradas para: errores 5xx > umbral, login failures > umbral, patron de SQL injection detectado. |
...
| 1.8.2 | Verificar que todos los niveles de proteccion tengan un conjunto asociado de requisitos de proteccion, como requisitos de cifrado, integridad, retencion, privacidad y otros requisitos de confidencialidad, y que estos se apliquen en la arquitectura | L2 | ⏳ | **Requisitos por nivel:** **CRITICO**: password_hash con Argon2id, JWT signing key en env vars, tokens con expiracion corta. **ALTO**: Cifrado en reposo (Aurora), TLS en transito, soft delete (nunca borrado fisico), retencion 7 anos (SAT Mexico). **MEDIO**: Cifrado en reposo, backup diario. **BAJO**: Backup diario. |
...
| ID | Requisito | Nivel | Estado | Medida Implementada |
|----|-----------|-------|--------|---------------------|
| 1.9.1 | Verificar que la aplicacion cifre las comunicaciones entre componentes, particularmente cuando esten en diferentes contenedores, sistemas, sitios o proveedores de nube | L2 | ✅ | **Implementacion:** Frontend -> Backend: HTTPS obligatorio (TLS 1.2+). Backend -> Aurora: SSL mode=require con validacion de certificado. Backend -> Twilio/SendGrid: HTTPS. WebSocket: WSS (WebSocket Secure). No hay comunicacion en texto plano. |
| 1.9.2 | Verificar que los componentes de la aplicacion verifiquen la autenticidad de cada lado en un enlace de comunicacion para prevenir ataques person-in-the-middle. Por ejemplo, los componentes de la aplicacion deben validar certificados y cadenas TLS | L2 | ✅ | **Implementacion:** Backend valida certificados TLS de Aurora y servicios externos (default JVM trust store + actualizaciones). JWTs firmados localmente con RS256, verificados con clave publica. HSTS header en respuestas (max-age=31536000). |
...
| ID | Requisito | Nivel | Estado | Medida Implementada |
|----|-----------|-------|--------|---------------------|
| 1.14.1 | Verificar la segregacion de componentes de diferentes niveles de confianza a traves de controles de seguridad bien definidos, reglas de firewall, gateways de API, reverse proxies, grupos de seguridad en la nube, o mecanismos similares | L2 | ✅ | **Implementacion:** Aurora BD: inaccesible desde internet publico, conectividad interna via VPC Endpoints. Frontend en AWS S3+CloudFront: solo assets estaticos, sin acceso a BD. Backend en AWS Lambda: unico punto de entrada a BD. WebSocket en mismo backend (misma autenticacion). |
| 1.14.2 | Verificar que firmas binarias, conexiones de confianza, y endpoints verificados se usen para desplegar binarios a dispositivos remotos | L2 | ⏳ | **Implementacion:** AWS Lambda pull del ZIP desde S3 / deploy automatizado via GitHub Actions (conexion autenticada). Dependencias verificadas via checksums en Maven/npm lockfiles. |
...
| 1.14.5 | Verificar que los despliegues de aplicacion esten adecuadamente sandboxed, containerizados y/o aislados a nivel de red para retrasar y disuadir atacantes de atacar otras aplicaciones, especialmente cuando realizan acciones sensibles o peligrosas como deserializacion | L2 | ✅ | **Implementacion:** Entorno aislado en AWS Lambda (microVM Firecracker). Network isolation por VPC. No hay shell access en produccion. Ejecuta como non-root. Read-only filesystem excepto /tmp. Recursos limitados (CPU, memoria, timeout) para prevenir DoS. |
