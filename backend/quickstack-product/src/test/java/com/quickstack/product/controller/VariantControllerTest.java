...
class VariantControllerTest {
        ...
        @BeforeEach
        void setUp() {
                principal = new JwtAuthenticationPrincipal(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                                null,
                                "manager@test.com");
        ...
        void listVariants_Success() throws Exception {
                VariantResponse res1 = new VariantResponse(UUID.randomUUID(), "Chico", "SKU1", BigDecimal.ZERO,
                                BigDecimal.TEN,
                                true, true, 1);
        ...
        void addVariant_Success() throws Exception {
                ...
                VariantResponse res = new VariantResponse(variantId, "Medio", "SKU2", BigDecimal.TEN,
                                new BigDecimal("20.00"),
                                false, true, 2);

                when(catalogPermissionEvaluator.canManageCatalog(any())).thenReturn(true);
                when(variantService.addVariant(eq(principal.tenantId()), eq(principal.userId()), eq(productId), any()))
                                .thenReturn(res);
        ...
        void addVariant_ForbiddenWithoutPermission() throws Exception {
                VariantCreateRequest req = new VariantCreateRequest("Medio", "SKU2", BigDecimal.TEN, false, 2);
                JwtAuthenticationPrincipal cashierPrincipal = new JwtAuthenticationPrincipal(UUID.randomUUID(),
                                UUID.randomUUID(), UUID.randomUUID(), null, "cashier@test.com");
        ...
        void updateVariant_Success() throws Exception {
                ...
                VariantResponse res = new VariantResponse(variantId, "Grande", "SKU-GR", new BigDecimal("20.00"),
                                new BigDecimal("30.00"), false, true, 3);

                when(catalogPermissionEvaluator.canManageCatalog(any())).thenReturn(true);
                when(variantService.updateVariant(eq(principal.tenantId()), eq(principal.userId()), eq(productId),
                                eq(variantId), any())).thenReturn(res);
